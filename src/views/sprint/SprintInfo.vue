  ...
  <div>
    <div class="mb15">
      <span style="display: inline-block;vertical-align: middle;"
        >{{ $t('负责人') }}：{{ sprintInfo.chargeUser }}</span
      >
      <span class="ml15 sprint-info" :title="sprintInfo.sprintDesc"
        >{{ $t('描述') }}：{{ sprintInfo.sprintDesc || '--' }}</span
      >
    </div>
    <div>
      <span
        ...
        <span class="progress-list">
          <el-progress
            :text-inside="true"
            :stroke-width="18"
            :percentage="sprintInfo.process"
          ></el-progress>
        </span>
      </span>
      <span class="ml15"
        >{{ $t('需求') }}({{ sprintInfo.completeRequires }}/{{
          sprintInfo.totalRequires
        }})</span
      >
      <span class="ml15"
        >{{ $t('任务') }}({{ sprintInfo.completeTasks }}/{{
          sprintInfo.totalTasks
        }})</span
      >
      <span class="ml15"
        >{{ $t('缺陷') }}({{ sprintInfo.closedDefect }}/{{
          sprintInfo.totalDefect
        }})</span
      >
      <span class="ml15"
        >{{ $t('任务工时') }}({{ sprintInfo.completeTaskHours }}/{{
          sprintInfo.totalTaskHours
        }})</span
      >
      <span class="ml15"
        >{{ $t('需求延期数') }}：<el-link
          ...
          >{{ sprintInfo.completeDelayRequires }}</el-link
        ></span
      >
      <span class="ml15"
        >{{ $t('任务延期数') }}：<el-link
          class="link"
          type="primary"
          @click="checkDelayList('task')"
          >{{ sprintInfo.completeDelayTasks }}</el-link
        ></span
      >
    </div>
      ...
      <div>
        ...
        >
          <div class="sprint-header-left">
            <span class="progress-title">{{ $t('当前阶段') }}</span>
            <span
              v-show="
                $authFunction(
          ...
          <div class="sprint-header-right">
            ...
            <el-link type="primary" :underline="false" @click="custormBtn"
        ...
        >
          ...
          >
            ...
            <div class="progress-item-content">
              <div class="progress-item-content-tag-box mb15">
