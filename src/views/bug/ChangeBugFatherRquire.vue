<template>
  ...
  >
    <el-form :inline="true" @submit.native.prevent>
      <el-form-item class="header-input" :label="`${$t('迭代')}:`">
        <op-group-select
          v-model="searchInfo.sprintIds"
...
export default {
  name: 'ChangeBugFatherRequire',
  components: {
    OpGroupSelect,
  },
  mixins: [FilterCommonMixin],
  props: {
  ...
  data() {
    return {
      ...
      localRequireId: this.requireId, // 需求id
      tableLoading: false,
      confirm: 0,
    }
  },
  ...
  methods: {
    ...
    async updateRequirement() {
      if (!this.localRequireId) {
        this.$message({ type: 'warning', message: this.$t('请选择所属需求') })
        return
      }
      ...
      // 如果有bugId，则是编辑模式，否则是新建页面
      if (this.bugId) {
        const res = await updateBugParentPost({
          id: this.bugId,
        ...
        const selectedRequire = this.requirementList.find(
          r => r.id === this.localRequireId,
        )
        this.$emit('onChange', selectedRequire)
        this.closeModal()
      }
    },
    // 选择需求 - 点击查询
    searchRequrieList({ pageNumber = DEFAULT_PAGE_NUMBER, pageSize } = {}) {
      const { pageInfo, title, sprintIds } = this.searchInfo
      ...
      parentAble({
        ...
        if (res.status === 200) {
          ...
          this.$message({
            message: res.msg || this.$t('没有满足条件的需求'),
            type: 'error',
          })
